...
//          TODO (2) Only implement the functionality, given the proper URI, to delete ALL rows in the weather table
        if (sUriMatcher.match(uri) != CODE_WEATHER && sUriMatcher.match(uri) != CODE_WEATHER_WITH_DATE) {
            return 0;
        }
        ...
        int rows = db.delete(WeatherContract.WeatherEntry.TABLE_NAME, selection, selectionArgs);
