...
//      TODO (7) Generate a weather summary with the date, description, high and low
        ...
        int idIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID);
        int hiIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP);
        int loIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP);

        long date = mCursor.getLong(dateIndex);
        int weatherId = mCursor.getInt(idIndex);

        summary.append(SunshineDateUtils.getFriendlyDateString(mContext, date, false));
        summary.append(" - ");
        summary.append(SunshineWeatherUtils.getStringForWeatherCondition(mContext, weatherId));
        summary.append(" - ");
        summary.append(SunshineWeatherUtils.formatHighLows(
                mContext,
                mCursor.getDouble(hiIndex),
                mCursor.getDouble(loIndex)
        ));

//      TODO (8) Display the summary that you created above
        forecastAdapterViewHolder.weatherSummary.setText(summary.toString());
    }
